package com.ias.assembly.zkpro.zk.http.json;
...
public class JsonUtil {
    ...
    public <K,V> Map<K,V> getJsonToMap(String json,Class<K> keyclazz,Class<V> valueclazz){
    	...
    	if(!StringUtils.isEmpty(json)) {
            try {
				JavaType javaType = mapper.getTypeFactory().constructParametrizedType(Map.class, Map.class, keyclazz,valueclazz);
	...
	public Object getJsonToObject(String json,Class<?> objclazz,Class<?> ...pclazz){
        ...
        if(!StringUtils.isEmpty(json)) {
            try {
            	JavaType javaType = mapper.getTypeFactory().constructParametrizedType(objclazz, objclazz, pclazz);
                object=getMapper().readValue(json,javaType);
            } catch (Exception e) {
            }
        }
